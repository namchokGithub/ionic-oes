...
export class ExamServiceService {
  ...
  insert_pers_item_bank(ib_code:string,ib_name:string,ib_sub_id:string,ib_stem_amount:number,ib_option_amount:number){
    return this.http.get('http://localhost:3000/pers/check_user/'+ib_code+'/'+ib_name+'/'+ib_sub_id+'/'+ib_stem_amount+'/'+ib_option_amount+'/1/1/1').pipe(map(res => res.json()))
  }
  ...
  update_pers_item_bank(ib_name:string,ib_stem_amount:string,ib_id:number){
    return this.http.get('http://localhost:3000/pers/update_pers_item_bank/'+ib_name+'/'+ib_stem_amount+'/'+ib_id).pipe(map(res => res.json()))
  }
